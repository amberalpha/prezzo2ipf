...
function(
  ...
  xsing <- x0[ntr<2]
  nminaug <- nminhard # *
  xs <- x1[, N := sum((ntr-1)), by = s][N >= nmin] #sum ntr-1 per id, (=nseg)
  xs[,level:='s']
  xd <- x1[, N := sum((ntr-1)), by = d][N >= nmin]
  xd[,level:='d']
  xda <- x1[ (N<nmin) & (nminaug<=N)]
  xda[,level:='da']
  xdrop <- x1[(N<nminaug)]
  xdrop[,level:='drop']
...
#' @export
streetaFun <-
function(nn='rppd',nppd=min(1E7,nrow(rppd)*.5)) {
  getgd(nn)
  tkn <- unlist(lapply(lapply(lapply(lapply(lapply(rppd[sample(nrow(rppd),nppd),street],strsplit,' '),unlist),'[',-1),rev),'[',1))
  streetad <<- table(tkn)
  putt(streetad)
}
...
function(nn='rppd') {
  getgd(nn)
  ch1d <<- rppd[,.(estate=min(estate_type),newbuild=min(new_build),type=min(property_type)),id]
  putt(ch1d)
}
...
function(
  ...
  ,
  cpu=min(ceiling(ncpus()*1.5),length(ii))
  ,
  parallel=T
  ,
  ...
  ll <- sfLapplyWrap(as.list(ii),stc1,cohod=cohod,prime1rcd=prime1rcd,tpxputd=tpxputd,zoned=zoned,setdad=setdad,deltatp=deltatp,deltasp=deltasp,startdad=startdad,deltatpce=deltatpce)
  sfStop()
...
ceroFun <-
function(
                    nn=c('celirawd','cncd','scoxd','refzoned')
                    ,
  ...
  if(rotate=='identity') { nn <- setdiff(nn,'scoxd') }
  getgd(nn)
...
# xx <- rr2(x1,tpd)
    xx <- rr2(scoxd,tpd)
    #browser()
    xx <- sweep(xx,MAR=2,STAT=flip*c(1,-sign(skewness((scoxd%*%xx)[,-1]))),FUN=`*`) #skew -ve apart from 1; flip applied again for 3
...
#' @export
ppm1Fun <-
function(nn=c('pvidd','ev2d','j1d',ch1d='ch1d')) {
  getgd(nn)
  x1 <- pvidd[,.(rcode=max(rcode),price_paid=max(price_paid),lastsale=max(lastsale),pv=max(pv),dfinal=max(dfinal)),id]
  x2 <- ev2d[,.(brn,median,gro)]
  x3 <- j1d[!is.na(brn),.(brn,a0,rc12,id)]
  ...
  ppm1d <<- ch1d[x4,on=c(id='id')][!is.na(median)] #not sure how na median gets here, but .024 are
  putt(ppm1d)
}
...
function(nn=c('celid','cncd','cnced'),...) {
  getgd(nn)
  #decd <- aappd::dec1(celid=gett('celid'),cncd=gett('cncd'),...)
  dec1d <- dec1(celid=gett('celid'),cncd=cncd,...)
  ...
  function(nn='pvidd') {
    ...
    xx <-
      pvidd[, .(
        ...
        trans = sum(lastsale>Sys.Date()-365)
      ), rcode][rcode != 'drop']
...
#' @export
dezojFun <-
function(nn=c('dezocombod','celid','scoxd')
                     ,
  ...
  xx <- dezocombod[, irf := (vafo - vafoj) / te][j, irp := 0][j, irf :=
                                                                    0] #irf, info raio of ex-ante premium
...
#' @export
lmregFun <-
function(nn=c('lmlhsd','lmrhsd','lmcend')){  #},pwr=4) {
  getgd(nn)
...
rdo4Fun <- function(nn=c('dezod','rdo2d')) {
  ...
  x1 <- rdo2d$d
  f1 <- function(x1,a1){
    x1[data.table(x1@data)[,substr(regpcode(name),1,3)%in%a1],]
  }
...
#this version infeasible because uses zomad
#' #weights for interpolation of holes - need to save because used dynamically in gui
#' #' @export
#' dezoint1Fun <- function(nn=c('distanced','zomad','dezojd','agpcod','cohod','zoned')){
#'   getgd(nn)
#'   x1 <- unique(zomad[datalevel!=maplevel&nchar(rc)==6,.(rc,datakey)]) #180 districts that take value from nearest zones, no longer from area
#'   x2 <-x1[,unique(rc)]
#'   x3 <- intersect(agpcod[,rc],x2) #just 2 are not : these are the missing districts
#'   x3a <- cohod[nchar(rc)==6,unique(rc)] #zones which are districts
#'   x4 <- union(x3,x3a) #because distance is a symmetric function, need both known and unknown
#'   zone1d <- setkey(agpcod[nchar(rc)==6],rc)[x4]
#'   zone1d
#'   d1 <- distance(zone1d,save=F) #distance from the missing districts x2 to the zones; rc in x2, other in x3a
#'   d2 <- setkey(d1,rc,other)[CJ(x2,x3a)]
#'   d2[,all(x2%in%rc)&all(x3a%in%other)&nrow(d2)==length(x2)*length(x3a)]
#'   dezoint1d <<- setkey(d2,rc,dist)[,rr:=0:(.N-1),rc][rr>0&rr<4][,w:=max(rr)-(rr-1),rc][,weight:=w/sum(w),rc][,w:=NULL]
#'   putt(dezoint1d)
#' }
...
#' #' @export
#' dezointFun <- function(nn=c('distanced','zomad','dezojd','agpcod','cohod','zoned')){
#'   getgd(nn)
#'   x1 <- unique(zomad[datalevel!=maplevel&nchar(rc)==6,.(rc,datakey)]) #180 districts that take value from nearest zones, no longer from area
#'   x2 <-x1[,unique(rc)]
#'   x3 <- intersect(agpcod[,rc],x2) #just 2 are not : these are the missing districts
...
#'   d2 <- setkey(d1,rc,other)[CJ(x2,x3a)]
#'   d2[,all(x2%in%rc)&all(x3a%in%other)&nrow(d2)==length(x2)*length(x3a)]
#'   x5 <- setkey(d2,rc,dist)[,rr:=0:(.N-1),rc][rr>0&rr<4][,w:=max(rr)-(rr-1),rc][,weight:=w/sum(w),rc][,w:=NULL]
#'   x6 <- dezojd[x5[,.(rc,other,weight)],on=c(zo='other')]
#'   x7 <- x6[,zo:=NULL][,as.data.table(lapply(.SD,weighted.mean,weight=weight)),rc][,weight:=NULL]
#'   x8 <- setnames(x7,old='rc',new='zo')
#'   dezointd <<- rbind(dezojd,x8)
#'   putt(dezointd)
#' }
